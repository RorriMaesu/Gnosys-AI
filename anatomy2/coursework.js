(function () {
    ...
    function showTypingIndicator() {
        ...
        wrap.innerHTML = `
            <div class="bg-slate-900 border border-slate-800 text-slate-400 rounded-2xl rounded-tl-none px-4 py-3 flex items-center space-x-1.5 shadow-md">
    ...
    async function loadLecture() {
        ...
        let cached = null;
        try {
            cached = JSON.parse(localStorage.getItem(`anatomy2_lesson_lectures_${appState.lesson.id}`));
        } catch (e) {
            console.error(e);
        }
        ...
        if (Array.isArray(cached) && cached.length > 0) {
            ...
            let savedIdx = Number(localStorage.getItem(`anatomy2_lesson_active_lecture_idx_${appState.lesson.id}`) || '0');
    ...
    async function generateLecture(lesson, index, isRegenerate = false) {
        ...
        const modelLabel = typeof window.getActiveModelLabel === 'function'
            ? window.getActiveModelLabel('anatomy2_llm')
            : (localStorage.getItem('anatomy2_llm') === 'llama' ? 'Gnosys Llama' : 'Gnosys Gemma');
        ...
        try {
            ...
            localStorage.setItem(`anatomy2_lesson_lectures_${lesson.id}`, JSON.stringify(appState.lectures));
            localStorage.setItem(`anatomy2_lesson_active_lecture_idx_${lesson.id}`, String(index));
    ...
    function loadSandbox() {
        if (els.sandboxStatusBadge) {
            const comp = localStorage.getItem(`anatomy2_sandbox_complete_${appState.lesson.id}`) === 'true';
    ...
    function setupEventListeners() {
        ...
        if (els.selectActiveLecture) {
            els.selectActiveLecture.addEventListener('change', () => {
                ...
                localStorage.setItem(`anatomy2_lesson_active_lecture_idx_${appState.lesson.id}`, String(idx));
        ...
        if (els.btnDeleteLecture) {
            els.btnDeleteLecture.addEventListener('click', () => {
                ...
                if (confirm(`Delete Lecture Variation ${appState.activeLectureIdx + 1}? This action cannot be undone.`)) {
                    ...
                    localStorage.setItem(`anatomy2_lesson_lectures_${appState.lesson.id}`, JSON.stringify(appState.lectures));
                    
                    // Adjust active index
                    if (appState.activeLectureIdx >= appState.lectures.length) {
                        appState.activeLectureIdx = appState.lectures.length - 1;
                    }
                    localStorage.setItem(`anatomy2_lesson_active_lecture_idx_${appState.lesson.id}`, String(appState.activeLectureIdx));
        ...
        if (modelSelect) {
            const savedModel = localStorage.getItem('anatomy2_llm') || 'gemma';
            const endpoint = localStorage.getItem("anatomy2_ollama_endpoint") || "http://localhost:11434";
            ...
            if (typeof window.populateModelSelector === 'function') {
                window.populateModelSelector(modelSelect, savedModel, cleanEndpoint, {
                    ...
                    onStatusChange: (status) => {
                        console.log('[Anatomy II Coursework Model Select]', status.message);
                    }
                });
            } else {
                ...
                modelSelect.addEventListener('change', () => {
                    localStorage.setItem('anatomy2_llm', modelSelect.value);
                });
            }
        }
        ...
        const updateBypassUis = () => {
            ...
            if (headerBypassBtn) {
                ...
                if (enabled) {
                    ...
                    headerBypassBtn.classList.remove('bg-rose-500/10', 'text-rose-300', 'border-rose-500/35');
                } else {
                    headerBypassBtn.classList.remove('bg-rose-500/25', 'text-rose-200', 'border-rose-500');
                    headerBypassBtn.classList.add('bg-rose-500/10', 'text-rose-300', 'border-rose-500/35');
                }
            }
        };
