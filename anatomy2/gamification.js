/**
 * Gnosys Anatomy & Physiology II (BI 232Z) - Gamification Engine
...
window.AnatomyGamification = (() => {
    ...
    // ==========================================
    const LEVELS = [
        { level: 1,  title: 'CNS Novice',                       xpRequired: 0     },
        { level: 2,  title: 'Autonomic Modulator',              xpRequired: 100   },
        { level: 3,  title: 'Special Sensationist',             xpRequired: 250   },
        { level: 4,  title: 'Endocrine Secretor',               xpRequired: 500   },
        { level: 5,  title: 'Erythrocyte Typer',                xpRequired: 850   },
        { level: 6,  title: 'Hemodynamic Editor',               xpRequired: 1300  },
        { level: 7,  title: 'Circulatory Cartographer',         xpRequired: 1850  },
        { level: 8,  title: 'Cardiovascular Expert',            xpRequired: 2500  },
        { level: 9,  title: 'Distinguished Physiologist',       xpRequired: 3500  }
    ];

    const ACHIEVEMENTS = [
        { id: 'first_socratic',    label: 'Neurological Dialogue', desc: 'Clear your first Socratic coursework session',      icon: 'fa-solid fa-comments',         xp: 30,  color: '#fb7185' },
        ...
        { id: 'habit_3',           label: 'Synaptic Habit',       desc: 'Maintain a 3-day active study streak',              icon: 'fa-solid fa-fire',             xp: 30,  color: '#f97316' },
        { id: 'week_warrior',      label: 'Circadian Control',    desc: 'Maintain a 7-day active study streak',              icon: 'fa-solid fa-fire-flame-curved', xp: 60,  color: '#ef4444' },
        { id: 'halfway_there',     label: 'Systemic Associate',   desc: 'Master 3/6 total curriculum lessons',            icon: 'fa-solid fa-user-graduate',    xp: 100, color: '#06b6d4' },
        { id: 'completionist',     label: 'Cardiology Scholar',   desc: 'Master all 6 A&P II curriculum lessons',            icon: 'fa-solid fa-crown',            xp: 300, color: '#fbbf24' }
    ];
    ...
    // ==========================================
    const XP_KEY = 'anatomy2_gamification_xp';
    const STREAK_KEY = 'anatomy2_gamification_streak';
    const ACHIEVEMENTS_KEY = 'anatomy2_gamification_achievements';
    const QUESTS_KEY = 'anatomy2_gamification_quests';
    const STATS_KEY = 'anatomy2_gamification_stats';
    ...
    function awardXP(amount, source, elementOrCoords) {
        ...
        window.dispatchEvent(new CustomEvent('anatomy2XPAwarded', {
    ...
    function checkStatsAchievements() {
        ...
        try {
            const matrix = JSON.parse(localStorage.getItem('anatomy2_masteryMatrix') || '{}');
            let masteredCount = 0;
    ...
    function unlockAchievement(id) {
        ...
        window.dispatchEvent(new CustomEvent('anatomy2AchievementUnlocked', { detail: achObj }));
    }
    ...
    function showToastNotification(message, colorCode) {
        const containerId = 'anatomy2-gamification-toast-container';
