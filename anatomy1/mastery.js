/**
 * BI 231Z Mastery State Manager & Syllabus Parser
...
const CURRICULUM_BYPASS_KEY = 'anatomy1_curriculum_bypass';
const PROGRESS_RESET_MARKER_KEY = 'anatomy1_progress_reset_at';
const PROGRESS_RESET_EVENT = 'anatomy1ProgressReset';
const MASTERY_MATRIX_KEY = 'anatomy1_masteryMatrix';
...
function exportProgressionJSON() {
    const backupData = {
        ...
        data: {
            ...
            'anatomy1_gamification_xp': localStorage.getItem('anatomy1_gamification_xp'),
            'anatomy1_gamification_streak': localStorage.getItem('anatomy1_gamification_streak'),
            'anatomy1_gamification_achievements': localStorage.getItem('anatomy1_gamification_achievements'),
            'anatomy1_gamification_quests': localStorage.getItem('anatomy1_gamification_quests'),
            'anatomy1_gamification_stats': localStorage.getItem('anatomy1_gamification_stats'),
    ...
    downloadAnchor.setAttribute("download", `anatomy1_progress_backup_${new Date().toISOString().slice(0,10)}.json`);
...
function importProgressionJSON(fileEvent) {
    ...
    fileReader.onload = function(e) {
        try {
            ...
            if (parsed.course !== 'anatomy1' || !parsed.data) {
                alert('Invalid backup file. Make sure this is an Anatomy & Physiology I (BI 231Z) backup.');
                return;
            }
...
function resetAnatomyProgressData() {
    const protectedKeys = new Set(['anatomy1_darkmode', 'anatomy1_llm']);
    const exactKeysToRemove = new Set([
        MASTERY_MATRIX_KEY,
        CURRICULUM_BYPASS_KEY,
        'anatomy1_gamification_xp',
        'anatomy1_gamification_streak',
        'anatomy1_gamification_achievements',
        'anatomy1_gamification_quests',
        'anatomy1_gamification_stats'
    ]);
    const prefixesToRemove = [
        'anatomy1_lesson_lectures_',
        'anatomy1_lesson_lecture_',
        'anatomy1_lesson_active_lecture_idx_',
        'anatomy1_sandbox_complete_'
    ];
...
function confirmAndResetAnatomyProgress() {
    const promptText = [
        'This will erase Anatomy & Physiology I progress across Study Dashboard, Coursework, and Homework Binder.',
...
async function loadSyllabus() {
    try {
        const response = await fetch('anatomy1_lesson_plan_BI231Z.md');
        ...
        syllabus.modules.forEach(mod => {
            ...
            <div class="border border-slate-850 rounded overflow-hidden">
                <button onclick="this.nextElementSibling.classList.toggle('hidden')" class="w-full bg-slate-950 hover:bg-slate-900 text-left px-3 py-2.5 flex items-center justify-between text-xs font-bold font-mono tracking-wider border-b border-slate-850 select-none">
                    <span class="text-rose-400 leading-tight">M${mod.number}: ${mod.title}</span>
