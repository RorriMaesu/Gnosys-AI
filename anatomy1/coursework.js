(function () {
    ...
    function setStageDots(currentStage) {
        for (let i = 1; i <= 4; i++) {
            ...
            if (i <= currentStage) {
                ...
                wrap.classList.add('text-rose-300', 'bg-rose-900/20', 'border', 'border-rose-950/40');
            } else {
    ...
    function setStage(stageNum) {
        ...
        if (stageNum === STAGE_LECTURE) {
            ...
            appState.socraticCollapsed = false;
            // Check if feynman has been loaded
    ...
    async function loadLecture() {
        ...
        let cached = null;
        try {
            cached = JSON.parse(localStorage.getItem(`anatomy1_lesson_lectures_${appState.lesson.id}`));
        } catch (e) {
            console.error(e);
        }
        ...
        if (Array.isArray(cached) && cached.length > 0) {
            ...
            let savedIdx = Number(localStorage.getItem(`anatomy1_lesson_active_lecture_idx_${appState.lesson.id}`) || '0');
    ...
    async function generateLecture(lesson, index, isRegenerate = false) {
        ...
        try {
            ...
            localStorage.setItem(`anatomy1_lesson_lectures_${lesson.id}`, JSON.stringify(appState.lectures));
            localStorage.setItem(`anatomy1_lesson_active_lecture_idx_${lesson.id}`, String(index));
            ...
            if (els.lectureContainer) {
                els.lectureContainer.innerHTML = `
                    <div class="p-6 text-center space-y-3">
                        ...
                        <p class="text-xs text-slate-450 max-w-sm mx-auto">
    ...
    function loadSocraticChat() {
        ...
        // Filter history by current mode or rebuild previous
    ...
    function loadSandbox() {
        if (els.sandboxStatusBadge) {
            const comp = localStorage.getItem(`anatomy1_sandbox_complete_${appState.lesson.id}`) === 'true';
    ...
    // This callback is called by skills-lab.js
    ...
    async function submitFeynmanExplanation(text) {
        ...
        try {
            ...
            if (res.passed) {
                ...
                // Transition lesson state to HW pending
    ...
    function setupEventListeners() {
        ...
        if (els.selectActiveLecture) {
            els.selectActiveLecture.addEventListener('change', () => {
                ...
                localStorage.setItem(`anatomy1_lesson_active_lecture_idx_${appState.lesson.id}`, String(idx));
        ...
        if (bypassToggleBtn) {
            bypassToggleBtn.addEventListener('click', () => {
                const next = window.toggleCurriculumBypass();
                updateBypassUis();
