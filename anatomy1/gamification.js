/**
 * Gnosys Anatomy & Physiology I (BI 231Z) - Gamification Engine
...
window.AnatomyGamification = (() => {
    ...
    // ==========================================
    const LEVELS = [
        { level: 1,  title: 'Somatic Novice',                  xpRequired: 0     },
        { level: 2,  title: 'Homeostatic Regulator',            xpRequired: 100   },
        { level: 3,  title: 'Cellular Transporter',             xpRequired: 250   },
        { level: 4,  title: 'Histological Sorter',              xpRequired: 500   },
        { level: 5,  title: 'Dermal Cartographer',              xpRequired: 850   },
        { level: 6,  title: 'Osteon Architect',                 xpRequired: 1300  },
        { level: 7,  title: 'Synovial Kinematician',            xpRequired: 1850  },
        { level: 8,  title: 'Excitable Membrane Biophysicist',  xpRequired: 2500  },
        { level: 9,  title: 'Distinguished Anatomist',          xpRequired: 3500  }
    ];

    const ACHIEVEMENTS = [
        { id: 'first_socratic',    label: 'Homeostatic Dialogue', desc: 'Clear your first Socratic coursework session',      icon: 'fa-solid fa-comments',         xp: 30,  color: '#fb7185' },
        ...
        { id: 'halfway_there',     label: 'Physiology Associate', desc: 'Master 4/7 total curriculum lessons',            icon: 'fa-solid fa-user-graduate',    xp: 100, color: '#06b6d4' },
        { id: 'completionist',     label: 'Anatomical Scholar',   desc: 'Master all 7 A&P I curriculum lessons',             icon: 'fa-solid fa-crown',            xp: 300, color: '#fbbf24' }
    ];
    ...
    // ==========================================
    const XP_KEY = 'anatomy1_gamification_xp';
    const STREAK_KEY = 'anatomy1_gamification_streak';
    const ACHIEVEMENTS_KEY = 'anatomy1_gamification_achievements';
    const QUESTS_KEY = 'anatomy1_gamification_quests';
    const STATS_KEY = 'anatomy1_gamification_stats';
    ...
    function awardXP(amount, source, elementOrCoords) {
        ...
        window.dispatchEvent(new CustomEvent('anatomy1XPAwarded', {
    ...
    function checkStatsAchievements() {
        ...
        try {
            const matrix = JSON.parse(localStorage.getItem('anatomy1_masteryMatrix') || '{}');
            let masteredCount = 0;
            Object.values(matrix).forEach(item => {
                if (item.state === 3) masteredCount++; // STATE_MASTERED
            });

            if (masteredCount >= 4) unlockAchievement('halfway_there');
            if (masteredCount >= 7) unlockAchievement('completionist');
        } catch (_err) {}
    }
    ...
    function unlockAchievement(id) {
        ...
        window.dispatchEvent(new CustomEvent('anatomy1AchievementUnlocked', { detail: achObj }));
    }
    ...
    function showToastNotification(message, colorCode) {
        const containerId = 'anatomy1-gamification-toast-container';
