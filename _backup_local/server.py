...
def get_gpu_info():
    # Try nvidia-smi — check multiple known paths because subprocess may not inherit full PATH
    nvidia_smi_candidates = [
        "nvidia-smi",
        r"C:\Windows\System32\nvidia-smi.exe",
        r"C:\Program Files\NVIDIA Corporation\NVSMI\nvidia-smi.exe",
    ]
    for smi_path in nvidia_smi_candidates:
        try:
            out = subprocess.check_output(
                [smi_path, "--query-gpu=name,memory.total", "--format=csv,noheader,nounits"],
                text=True,
    ...
    # Fallback: registry QWORD read on Windows (avoids 32-bit AdapterRAM overflow in WMI)
    if platform.system() == 'Windows':
        try:
            # HardwareInformation.MemorySize is a 64-bit QWORD — no overflow for large VRAM GPUs
            ps_script = (
                ...
                '  $key = $_;'
                '  $name = (Get-ItemProperty -Path $key.PSPath -Name "DriverDesc" -ErrorAction SilentlyContinue).DriverDesc;'
                '  $memBytes = (Get-ItemProperty -Path $key.PSPath -Name "HardwareInformation.MemorySize" -ErrorAction SilentlyContinue)."HardwareInformation.MemorySize";'
                '  if ($name -and $memBytes) {'
                '    $result += [PSCustomObject]@{Name=$name; MemoryBytes=[int64]$memBytes}'
                '  }'
                '};'
                '$result | ConvertTo-Json'
            )
            cmd = f'powershell -Command "{ps_script}"'
            out = subprocess.check_output(cmd, shell=True, text=True, stderr=subprocess.DEVNULL, timeout=8)
            data = json.loads(out)
            ...
            for item in data:
                ...
                name = item.get("Name", "")
                mem_bytes = item.get("MemoryBytes", 0) or 0
        ...
        # Last resort: WMI AdapterRAM (known 32-bit overflow for >=4GB multiples, but better than nothing)
        try:
