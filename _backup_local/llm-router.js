(function () {
    ...
    async function showDesktopOllamaLaunchModal() {
        ...
        // Clean raw WebGPU/ANGLE GPU name strings:
        //   "ANGLE (NVIDIA ..., Direct3D11 ...)" → inner name
        //   "NVIDIA GeForce RTX 5060 Ti (0x00002D04) Direct3D11 vs_5_0 ps_5_0, D3D11" → clean name
        let gpuDisplayStr = hw.gpuName
            .replace(/^ANGLE \(([^,]+),.*\)$/, '$1')          // ANGLE wrapper
            .replace(/\s*\(0x[0-9a-fA-F]+\).*$/i, '')          // hex device ID + trailing junk
            .replace(/,\s*Direct3D.*$/i, '')                    // remaining Direct3D suffix
            .trim();
        if (hw.vramGb) {
            gpuDisplayStr += ` (${hw.vramGb}GB VRAM)`;
        } else if (hw.hasGamingGpu) {
            gpuDisplayStr += ` (VRAM: N/A)`;
        }
