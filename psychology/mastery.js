/**
 * PSY 201Z Mastery State Manager & Syllabus Parser
...
const CURRICULUM_BYPASS_KEY = 'psychology_curriculum_bypass';
const PROGRESS_RESET_MARKER_KEY = 'psychology_progress_reset_at';
const PROGRESS_RESET_EVENT = 'psychologyProgressReset';
const MASTERY_MATRIX_KEY = 'psychology_masteryMatrix';
...
function resetPsychologyProgressData() {
    const protectedKeys = new Set(['psych_darkmode', 'psychology_llm']);
    const exactKeysToRemove = new Set([
        MASTERY_MATRIX_KEY,
        CURRICULUM_BYPASS_KEY,
        'psychology_learning_state_v1',
        'psychology_video_state_v1'
    ]);
    const prefixesToRemove = [
        'psychology_lesson_lectures_',
        'psychology_lesson_lecture_',
        'psychology_lesson_active_lecture_idx_',
        'psychology_sandbox_complete_'
    ];
...
function showPsychologyResetToast(message) {
    const id = 'psychology-reset-toast';
    const existing = document.getElementById(id);
    if (existing) existing.remove();

    const toast = document.createElement('div');
    toast.id = id;
    toast.className = 'fixed right-4 top-4 z-[9999] max-w-xs rounded-lg border border-purple-500/35 bg-purple-500/10 px-3 py-2 text-xs font-semibold text-purple-800 shadow-lg backdrop-blur dark:border-purple-500/40 dark:bg-purple-500/15 dark:text-purple-200';
...
function schedulePsychologyResetReload(message, delayMs = 900) {
    if (window.__psychologyResetReloadScheduled) return;
    window.__psychologyResetReloadScheduled = true;
    showPsychologyResetToast(message || 'Progress reset accepted. Reloading...');
    window.setTimeout(() => {
        window.location.reload();
    }, delayMs);
}

function confirmAndResetPsychologyProgress() {
    const promptText = [
        'This will erase psychology progress across Study Dashboard, Coursework, and Homework Binder.',
    ...
    const ok = resetPsychologyProgressData();
    if (ok) {
        schedulePsychologyResetReload('Progress reset accepted. Reloading module...');
    }
    return ok;
}
 ...
 * If not existing, sets all to 0 except lesson_1_1 which defaults to 1.
 */
...
/**
 * Calculates and updates UI elements representing progress.
 */
...
function getHighestUnlockedLesson(matrix, syllabus) {
    ...
    // Fallback to highest unlocked lesson
...
/**
 * Reads psychology_lesson_plan_PSY201Z.md, parses modules and lessons, & maps details.
 */
async function loadSyllabus() {
    try {
        const response = await fetch('psychology_lesson_plan_PSY201Z.md');
    ...
    switch (state) {
        ...
        case STATE_ACTIVE:
            return {
                ...
                btnClass: 'text-purple-400 hover:bg-slate-850 bg-slate-850 p-2.5 rounded border border-purple-500/20 flex items-center justify-between shadow-inner',
                badgeText: 'ACTIVE',
                badgeClass: 'bg-purple-500/10 border border-purple-500/30 text-purple-500'
            };
        ...
        syllabus.modules.forEach(mod => {
            ...
            <div class="border border-slate-850 rounded overflow-hidden">
                <button onclick="this.nextElementSibling.classList.toggle('hidden')" class="w-full bg-slate-950 hover:bg-slate-900 text-left px-3 py-2.5 flex items-center justify-between text-xs font-bold font-mono tracking-wider border-b border-slate-850 select-none">
                    <span class="text-purple-400 leading-tight">M${mod.number}: ${mod.title}</span>
            ...
            lessons.forEach(l => {
                ...
                html += `
                <button 
                    onclick="if(${canSelectLesson}) { selectLesson('${l.id}') } else { alert('This psychology lecture is currently locked. Complete previous courseworks first!') }" 
    ...
    if (targetAs) {
        ...
        syllabus.modules.forEach(mod => {
            ...
            lessons.forEach(l => {
                ...
                html += `
                <button 
                    onclick="if(!${isHwLocked}) { selectAssignment('${l.id}') } else { alert('Theory not qualified. You must pass this lesson\\'s Socratic lecture and its Sandbox mapping before testing psychology concepts!') }" 
                    class="${buttonClass}" 
...
function highlightActiveInSidebar() {
    ...
    if (cwItem) {
        cwItem.classList.add('ring-1', 'ring-purple-500', 'bg-slate-800/80');
    }
...
window.resetPsychologyProgressData = resetPsychologyProgressData;
window.confirmAndResetPsychologyProgress = confirmAndResetPsychologyProgress;
window.PROGRESS_RESET_EVENT = PROGRESS_RESET_EVENT;
window.showPsychologyResetToast = showPsychologyResetToast;
window.schedulePsychologyResetReload = schedulePsychologyResetReload;
