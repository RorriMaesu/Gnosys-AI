/**
 * Gnosys PSY 201Z - Psychology Gamification Engine
 * Manages XP progression, study streaks, achievements, daily quests, and stats.
 */

window.PsychGamification = (() => {
    // ==========================================
    // GAMIFICATION CONSTANTS
    // ==========================================
    const LEVELS = [
        { level: 1,  title: 'Behavioral Apprentice',       xpRequired: 0     },
        { level: 2,  title: 'Psychoanalytic Introspector',  xpRequired: 100   },
        { level: 3,  title: 'Neuro-Synaptic Navigator',     xpRequired: 250   },
        { level: 4,  title: 'Sensory Receptor',             xpRequired: 500   },
        { level: 5,  title: 'Developmental Guardian',       xpRequired: 850   },
        { level: 6,  title: 'Conditioning Architect',       xpRequired: 1300  },
        { level: 7,  title: 'Cognitive Mapper',             xpRequired: 1850  },
        { level: 8,  title: 'Schema Builder',               xpRequired: 2500  },
        { level: 9,  title: 'Motivational Balancer',        xpRequired: 3250  },
        { level: 10, title: 'Emotion Appraiser',            xpRequired: 4100  },
        { level: 11, title: 'Personality Profiler',          xpRequired: 5050  },
        { level: 12, title: 'Attributional Analyst',        xpRequired: 6100  },
        { level: 13, title: 'Social Facilitator',           xpRequired: 7250  },
        { level: 14, title: 'Clinical Diagnostician',       xpRequired: 8500  },
        { level: 15, title: 'Psychopharmacologist',         xpRequired: 9900  },
        { level: 16, title: 'Socratic Master',              xpRequired: 11500 },
        { level: 17, title: 'APA Fellow',                   xpRequired: 13300 }
    ];

    const ACHIEVEMENTS = [
        { id: 'first_socratic',    label: 'Dialectic Explorer',  desc: 'Clear your first Socratic coursework session',      icon: 'fa-solid fa-comments',         xp: 30,  color: '#c084fc' },
        { id: 'first_sandbox',     label: 'Simulation Chemist',  desc: 'Resolve your first interactive sandbox simulation',  icon: 'fa-solid fa-cube',             xp: 30,  color: '#60a5fa' },
        { id: 'first_feynman',     label: 'Feynman Defense',     desc: 'Submit a passing Feynman Technique explanation',    icon: 'fa-solid fa-graduation-cap',    xp: 40,  color: '#34d399' },
        { id: 'first_mastery',     label: 'Accredited Step',     desc: 'Achieve Mastery (>=80%) on a Homework Quiz sheet',   icon: 'fa-solid fa-file-signature',   xp: 50,  color: '#f59e0b' },
        { id: 'perfect_quiz',      label: 'APA Scholar',         desc: 'Score a perfect 100% on any Homework Quiz sheet',    icon: 'fa-solid fa-award',            xp: 50,  color: '#fb7185' },
        { id: 'habit_3',           label: 'Cognitive Cohort',    desc: 'Maintain a 3-day active study streak',              icon: 'fa-solid fa-fire',             xp: 30,  color: '#f97316' },
        { id: 'week_warrior',      label: 'Operant Habit',       desc: 'Maintain a 7-day active study streak',              icon: 'fa-solid fa-fire-flame-curved', xp: 60,  color: '#ef4444' },
        { id: 'monthly_devotion',  label: 'Self-Actualized',     desc: 'Maintain a 30-day active study streak',             icon: 'fa-solid fa-calendar-check',    xp: 150, color: '#a855f7' },
        { id: 'halfway_there',     label: 'Psychology Associate', desc: 'Master 18/36 total curriculum lessons',            icon: 'fa-solid fa-user-graduate',    xp: 100, color: '#06b6d4' },
        { id: 'completionist',     label: 'Doctor of Psychology', desc: 'Master all 36 introductory psychology lessons',     icon: 'fa-solid fa-crown',            xp: 300, color: '#fbbf24' }
    ];
    ...
    const QUESTS = [
        ...
        { id: 'quest_video',      label: 'Complete 1 Crash Course episode',      target: 1, xp: 30, field: 'videosWatched' }
    ];

    // ==========================================
    // ENGINE STATE & LOCAL STORAGE KEYS
    // ==========================================
    const XP_KEY = 'psychology_gamification_xp';
    const STREAK_KEY = 'psychology_gamification_streak';
    const ACHIEVEMENTS_KEY = 'psychology_gamification_achievements';
    const QUESTS_KEY = 'psychology_gamification_quests';
    const STATS_KEY = 'psychology_gamification_stats';
    ...
    function verifyStreak() {
        ...
        if (streakData.count >= 30) unlockAchievement('monthly_devotion');
    }
    ...
    function awardXP(amount, source, elementOrCoords) {
        ...
        if (activeLevel > xpData.level) {
            ...
            levelChanged = true;
            setTimeout(() => {
                showToastNotification(`🎉 Level Up! You reached Level ${activeLevel}: ${LEVELS[activeLevel-1].title}`, '#a855f7');
            }, 800);
        }
        ...
        window.dispatchEvent(new CustomEvent('psychologyXPAwarded', {
    ...
    function checkStatsAchievements() {
        ...
        // Check coursework syllabus counts
        try {
            const matrix = JSON.parse(localStorage.getItem('psychology_masteryMatrix') || '{}');
            let masteredCount = 0;
            Object.values(matrix).forEach(item => {
                if (item.state === 3) masteredCount++; // STATE_MASTERED
            });

            if (masteredCount >= 18) unlockAchievement('halfway_there');
            if (masteredCount >= 36) unlockAchievement('completionist');
        } catch (_err) {}
    }
    ...
    function unlockAchievement(id) {
        ...
        window.dispatchEvent(new CustomEvent('psychologyAchievementUnlocked', { detail: achObj }));
    }
    ...
    function showToastNotification(message, colorCode) {
        const containerId = 'psychology-gamification-toast-container';
        ...
        toast.style.borderColor = colorCode || '#a855f7';
        toast.innerHTML = `
            <div class="p-1.5 rounded-full shrink-0 text-slate-900" style="background-color: ${colorCode || '#a855f7'}">
    ...
    function syncHeaderUI() {
        // Sync level title, level value, XP bar progress, and streak display in standard page headers
