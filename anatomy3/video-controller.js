(function () {
    const VIDEO_STATE_KEY = 'anatomy3_video_state_v1';
    ...
    function getEpisodes() {
        return Array.isArray(window.AnatomyVideoData?.videoCurriculum)
            ? window.AnatomyVideoData.videoCurriculum
            : [];
    }
    ...
    function buildVideoQuizSystemPrompt(episode) {
        ...
        return [
            'You are AnatomyTutor, a Socratic Anatomy & Physiology III tutor.',
            `The learner just finished video episode ${episode?.episodeNumber || ''}: ${episode?.title || 'Unknown'}.`,
            `Focus only on these A&P concepts: ${conceptText}.`,
            'Run a brief 3-question micro-quiz with one question at a time.',
            'After each answer, provide short Socratic feedback.',
            'Use plain text only.'
        ].join(' ');
    }
    ...
    function bindVideoActions() {
        ...
        const iframeEl = document.getElementById('anatomy-video-player');
        ...
        dashboardVideosBtn?.addEventListener('click', () => window.activateAnatomyTab?.('videos'));
        dashboardTutorBtn?.addEventListener('click', () => window.activateAnatomyTab?.('tutor'));

        const episodes = getEpisodes();
        if (episodes.length === 0) {
            listEl.innerHTML = '<div class="text-xs text-slate-500 p-3">No video curriculum entries found.</div>';
            return;
        }
        ...
        function setEpisodeCompletion(episodeId, shouldComplete, sourceEl) {
            ...
            if (shouldComplete) {
                ...
                if (window.AnatomyGamification) {
                    window.AnatomyGamification.awardXP?.(30, 'video', sourceEl || null);
                    window.AnatomyGamification.incrementStat?.('videosWatched', 1);
                }
            } else {
        ...
        function updateDashboardCta() {
            ...
            if (!active) {
                dashboardSummaryEl.textContent = 'Pick your next anatomy episode, then lock in concepts with a 3-question micro-quiz.';
                return;
            }
        ...
        // Adapted for Pink Theme class mapping
        function renderEpisodeList() {
            ...
            episodes.forEach((episode) => {
                ...
                row.className = [
                    ...
                    active ? 'bg-pink-950/20 border-pink-850' : 'bg-slate-900 hover:bg-slate-850/60 border-slate-800'
                ].join(' ');

                openBtn.type = 'button';
                openBtn.className = 'flex-1 text-left rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-pink-500/40';
                openBtn.innerHTML = `
                ...
                toggleBtn.className = [
                    ...
                    completed
                        ? 'bg-pink-900/25 text-pink-200 border-pink-700 hover:bg-pink-900/45'
        ...
        function renderActiveEpisode() {
            ...
            (episode.coreConcepts || []).forEach((concept) => {
                ...
                chip.className = 'px-2.5 py-1 rounded-full text-xs font-semibold bg-pink-900/25 border border-pink-800/40 text-pink-300';
        ...
        function launchMicroQuiz() {
            const episode = episodes.find((ep) => ep.id === activeEpisodeId);
            if (!episode) return;
            if (!window.AnatomyTutor || typeof window.AnatomyTutor.invoke !== 'function') return;

            const starterPrompt = `I just finished Video Episode ${episode.episodeNumber}: ${episode.title}. Please run a short anatomy micro-quiz focused on: ${(episode.coreConcepts || []).join(', ')}.`;
            ...
            if (mobile) {
                ...
                clearVideoQuizWidgets();
                window.AnatomyTutor.invoke(starterPrompt, quizMobileHost || tutorAnchor, systemContext, {
                    mountMode: 'append',
                ...
                iframeEl.classList.add('video-player-frame-hidden');
                clearVideoQuizWidgets();
                window.AnatomyTutor.invoke(starterPrompt, quizOverlayHost || tutorAnchor, systemContext, {
                    mountMode: 'append',
