/**
 * BI 233Z Mastery State Manager & Syllabus Parser
...
const CURRICULUM_BYPASS_KEY = 'anatomy3_curriculum_bypass';
const PROGRESS_RESET_MARKER_KEY = 'anatomy3_progress_reset_at';
const PROGRESS_RESET_EVENT = 'anatomy3ProgressReset';
const MASTERY_MATRIX_KEY = 'anatomy3_masteryMatrix';
...
function exportProgressionJSON() {
    const backupData = {
        ...
        data: {
            ...
            'anatomy3_gamification_xp': localStorage.getItem('anatomy3_gamification_xp'),
            'anatomy3_gamification_streak': localStorage.getItem('anatomy3_gamification_streak'),
            'anatomy3_gamification_achievements': localStorage.getItem('anatomy3_gamification_achievements'),
            'anatomy3_gamification_quests': localStorage.getItem('anatomy3_gamification_quests'),
            'anatomy3_gamification_stats': localStorage.getItem('anatomy3_gamification_stats'),
    ...
    downloadAnchor.setAttribute("download", `anatomy3_progress_backup_${new Date().toISOString().slice(0,10)}.json`);
...
function importProgressionJSON(fileEvent) {
    ...
    fileReader.onload = function(e) {
        try {
            ...
            if (parsed.course !== 'anatomy3' || !parsed.data) {
                alert('Invalid backup file. Make sure this is an Anatomy & Physiology III (BI 233Z) backup.');
                return;
            }
...
function resetAnatomyProgressData() {
    const protectedKeys = new Set(['anatomy3_darkmode', 'anatomy3_llm']);
    const exactKeysToRemove = new Set([
        MASTERY_MATRIX_KEY,
        CURRICULUM_BYPASS_KEY,
        'anatomy3_gamification_xp',
        'anatomy3_gamification_streak',
        'anatomy3_gamification_achievements',
        'anatomy3_gamification_quests',
        'anatomy3_gamification_stats'
    ]);
    const prefixesToRemove = [
        'anatomy3_lesson_lectures_',
        'anatomy3_lesson_lecture_',
        'anatomy3_lesson_active_lecture_idx_',
        'anatomy3_sandbox_complete_'
    ];
...
function confirmAndResetAnatomyProgress() {
    const promptText = [
        'This will erase Anatomy & Physiology III progress across Study Dashboard, Coursework, and Homework Binder.',
...
// Function helper to get the highest unlocked lesson to auto-select
...
async function loadSyllabus() {
    try {
        const response = await fetch('anatomy3_lesson_plan_BI233Z.md');
    ...
    if (targetAs) {
        ...
        syllabus.modules.forEach(mod => {
            ...
            lessons.forEach(l => {
                ...
                if (isHwLocked) {
                    buttonClass += 'text-slate-500 bg-slate-900/40 border border-slate-955 cursor-not-allowed';
                    badgeTxt = 'LOCKED';
